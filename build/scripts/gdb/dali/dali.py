...
# ====================
# Prints Dali::Vector, property types, handles and actors, plus some internals
   ...
   class _iterator(Iterator):
       ...
       def __next__(self):
           ...
           if elt.type.code == gdb.TYPE_CODE_PTR:
               if elt != 0:
                   # Tried being nice, and de-reffing ptrs automatically, but this leads to circular loop
                   # in observer lists, so don't do it.
                   #
...
class DaliHandlePrinter:
    ...
    def to_string (self):
        if self.obj.dynamic_type != 'WeakHandleBase':
...
class DaliActorPrinter:
    ...
    def __init__(self, typename, val):
        ...
        self.val = self.obj.cast(gdb.lookup_type("Dali::Internal::Actor").pointer())
...
def build_libdali_dictionary ():
    ...
    libdali_printer.add('Dali::Material',  DaliMaterialPrinter)
    libdali_printer.add('Dali::Material',  DaliMaterialPrinter)
