...
int UtcDaliEntityDataMakeAndFocusedActorP(void)
{
  ...
  DALI_TEST_EQUALS(registeredFocusedEntityData.actorId, std::to_string(actor.GetId()), TEST_LOCATION);
  ...
  DALI_TEST_EQUALS(focusedEntityData.actorId, std::to_string(actor.GetId()), TEST_LOCATION);
  DALI_TEST_EQUALS(focusedEntityData.actorTypeName, actor.GetTypeName().CStr(), TEST_LOCATION);
...
int UtcDaliEntityDataFindByActorIdP(void)
{
  ...
  DALI_TEST_CHECK(service.FindByActorId(std::to_string(actor.GetId()), root, entityData));
  DALI_TEST_EQUALS(entityData.actorId, std::to_string(actor.GetId()), TEST_LOCATION);
  ...
  DALI_TEST_CHECK(!service.FindByActorId(std::to_string(actor.GetId()), Actor(), entityData));
...
int UtcDaliEntityDataGetAnnotatedEntitiesP(void)
{
  ...
  DALI_TEST_CHECK(service.GetAnnotatedEntities(application.GetScene().GetRootLayer(), Dali::Bounds{0.0f, 0.0f, 480.0f, 800.0f}, entities));
  ...
  DALI_TEST_EQUALS(entities[1].actorId, std::to_string(first.GetId()), TEST_LOCATION);

  DALI_TEST_CHECK(service.GetAnnotatedEntities(Actor(), Dali::Bounds{0.0f, 0.0f, 480.0f, 800.0f}, entities));
...
int UtcDaliEntityDataPresentationValidationP(void)
{
  ...
  entityData.description.push_back('\t');
  entityData.screenBounds = Dali::Bounds{1.5f, 2.25f, 30.5f, 40.75f};
