...
function ConvertTo-XmlSafeText
{
  ...
  $WithoutAnsi = $Text -replace ($Escape + '\[[0-?]*[ -/]*[@-~]'), ""
  return $WithoutAnsi -replace '[\x00-\x08\x0B\x0C\x0E-\x1F]', ""
}
...
if($exitCode -ne 0)
{
  throw "Failed to list DALi adaptor common UTCs on Windows (exit code $exitCode).`n$stderr"
}
...
try
{
  ...
  $Writer.WriteAttributeString("name", "dali-adaptor-common-windows")
  ...
  foreach($Result in $Results)
  {
    ...
    $Writer.WriteAttributeString("classname", "dali-adaptor.common.windows")
    ...
    if($Result.Skipped)
    {
      ...
      $Writer.WriteAttributeString("message", $XmlMessage)
...
Write-Host "`nDALi adaptor common UTC summary on Windows: $PassedCount passed, $FailedCount failed, $SkippedCount skipped."
