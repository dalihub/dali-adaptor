 ...
 * @SINCE_1_0.0
 */
typedef Dali::IntrusivePtr<Dali::NativeImage> NativeImagePtr;

/**
 * @brief Used for displaying native images.
 *
 * NativeImage can be created internally or
 * externally by native image.
 ...
 * @see NativeImage
 */
class DALI_ADAPTOR_API NativeImage : public NativeImageInterface
{
public:
  /**
  ...
  /**
   * @brief Creates a new NativeImage.
   *
   ...
   */
  static NativeImagePtr New(uint32_t width, uint32_t height, ColorDepth depth);

  /**
   * @brief Creates a new NativeImage from an existing native image.
   *
   * @SINCE_1_0.0
   * @param[in] nativeImage NativeImage must be a any handle with native image
   * @return A smart-pointer to a newly allocated image
   * @see NativeImageInterface
   */
  static NativeImagePtr New(Any nativeImage);
  ...
  /**
   * @brief Gets a copy of the pixels used by NativeImage.
   *
...
private:
   ...
   * @param[in] height The height of the image
   * @param[in] depth color depth of the image
   * @param[in] nativeImage contains either: native image or is empty
   */
  DALI_INTERNAL NativeImage(uint32_t width, uint32_t height, ColorDepth depth, Any nativeImage);
   ...
   * @SINCE_1_0.0
   */
  DALI_INTERNAL ~NativeImage() override;
   ...
   * @SINCE_1_0.0
   * @param[in] nativeImage A reference to the object to copy
   */
  DALI_INTERNAL NativeImage(const NativeImage& nativeImage);
   ...
   */
  DALI_INTERNAL NativeImage& operator=(const NativeImage& rhs);
  /// @endcond

private:
  /// @cond internal
  Internal::Adaptor::NativeImage* mImpl; ///< Implementation pointer
  friend class Internal::Adaptor::NativeImage;
