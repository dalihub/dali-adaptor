#ifndef DALI_INTERNAL_WINDOWSYSTEM_TIZENWAYLAND_WINDOW_BASE_TCORE_WL_H
#define DALI_INTERNAL_WINDOWSYSTEM_TIZENWAYLAND_WINDOW_BASE_TCORE_WL_H
...
class WindowRenderSurfaceTcoreWl;

/**
 * WindowBaseTcoreWl class provides an WindowBase Tizen-Core-Wayland implementation.
 */
class WindowBaseTcoreWl : public WindowBase
{
public:
  /**
   * @brief Constructor
   */
  WindowBaseTcoreWl(PositionSize positionSize, Any surface, bool isTransparent);

  /**
   * @brief Destructor
   */
  virtual ~WindowBaseTcoreWl();

public:
  /**
   * @brief Called when the window iconify state is changed.
   */
  bool OnIconifyStateChanged(void* data, int type, void* event);
  ...
  bool OnOutputTransformChanged(void* data, int type, void* event);

  /**
   * @brief Called when the output is transformed.
   */
  bool OnOutputTransform(void* data, int type, void* event);

  /**
   * @brief Called when the output transform should be ignored.
   */
  bool OnIgnoreOutputTransform(void* data, int type, void* event);
  ...
  void OnWindowRotationChange(void* data, int type, void* event);
  ...
  /**
   * @brief Called when a mouse frame is received (Tizen path).
   */
  void OnMouseFrame(void* data, int type, void* event);

  /**
   * @brief Called when a relative mouse motion is received.
   */
   ...
   */
  void OnMouseInOut(void* data, int type, void* event);
   ...
   */
  void OnConformantChange(void* data, int type, void* event);
   ...
   */
  void OnWindowAuxiliaryMessage(void* data, int type, void* event);
   ...
   */
  void OnMoveCompleted(void* data, int type, void* event);
   ...
   * @param[in] the completed event's data. It has the latest window geometry data.
   */
  void OnResizeCompleted(void* data, int type, void* event);

  /**
   * @brief Called when a keymap is changed (Tizen path).
   */
...
private:
   ...
   * It is used when window is moved or resized for native tcore wayland window system.
   *
...
private:
  ...
  std::vector<tizen_core_wl_event_listener_h> mTcoreEventListeners;
  tizen_core_wl_display_h                     mTcoreDisplay;
  tizen_core_wl_window_h                      mTcoreWindow;
  tizen_core_wl_region_h                      mDisplayRegion;
  ...
  InsetsGeometry mLastIndicatorGeometry;
  InsetsGeometry mLastKeyboardGeometry;
  InsetsGeometry mLastClipboardGeometry;
  ...
  bool mIsFloating;
  bool mIsAlwaysTop;
  bool mPendingRestoreResizeOnUnmaximize;
  bool mSetRestoreWindowUnmaximize;

  WindowStateStatus mWindowStateStatus;
  int               mRestoreWindowWidth;
  int               mRestoreWindowHeight;
};
...
#endif // DALI_INTERNAL_WINDOWSYSTEM_TIZENWAYLAND_WINDOW_BASE_TCORE_WL_H
