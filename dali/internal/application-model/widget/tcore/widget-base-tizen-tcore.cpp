 ...
 public:
  class Factory : public tizen_cpp::AppCoreMultiWindowBase::Context::IFactory {
   public:
    ...
    {
      DALI_LOG_RELEASE_INFO("WidgetContext::Factory::Create called (instanceId:%s, app:%p)\n", instanceId.c_str(), app);
  ...
  WidgetContext(std::string contextId, std::string instanceId, tizen_cpp::AppCoreMultiWindowBase* app, Dali::Internal::Adaptor::WidgetApplicationTizen* application)
     : tizen_cpp::WidgetContext(contextId, instanceId, app),
       mApplication(application)
  {
    DALI_LOG_RELEASE_INFO("WidgetContext::WidgetContext called (contextId:%s, instanceId:%s, app:%p, application:%p)\n", contextId.c_str(), instanceId.c_str(), app, application);
  }

  bool EnsureApplication(const char* functionName)
  {
    DALI_LOG_RELEASE_INFO("WidgetContext::EnsureApplication called (function:%s, application:%p)\n", functionName, mApplication);
    if(!mApplication)
    {
      DALI_LOG_ERROR("WidgetContext::%s: mApplication is null", functionName);
      return false;
    }
  ...
  {
    DALI_LOG_RELEASE_INFO("WidgetContext::OnCreate called (w:%d, h:%d)\n", w, h);
    if(!EnsureApplication(__FUNCTION__))
    {
      DALI_LOG_ERROR("WidgetContext::OnCreate: EnsureApplication failed");
      return false;
    }

    std::string id = GetContextId();
    ...
    if(!bindingResult.ok)
    {
      DALI_LOG_ERROR("Widget Instance bind failed(id:%s): %s", std::string(id).c_str(), bindingResult.reason.c_str());
    }
    else
    {
      DALI_LOG_RELEASE_INFO("Widget Instance bind success(id:%s)\n", std::string(id).c_str());
    }
  ...
  {
    DALI_LOG_RELEASE_INFO("WidgetContext::OnDestroy called (reason:%d)\n", static_cast<int>(reason));
    if(!EnsureApplication(__FUNCTION__))
    {
      DALI_LOG_ERROR("WidgetContext::OnDestroy: EnsureApplication failed");
      return;
    }
  ...
  {
    DALI_LOG_RELEASE_INFO("WidgetContext::OnPause called\n");
    if(!EnsureApplication(__FUNCTION__))
    {
      DALI_LOG_ERROR("WidgetContext::OnPause: EnsureApplication failed");
      return;
    }
  ...
  {
    DALI_LOG_RELEASE_INFO("WidgetContext::OnResume called\n");
    if(!EnsureApplication(__FUNCTION__))
    {
      DALI_LOG_ERROR("WidgetContext::OnResume: EnsureApplication failed");
      return;
    }
  ...
  {
    DALI_LOG_RELEASE_INFO("WidgetContext::OnResize called (w:%d, h:%d)\n", w, h);
    if(!EnsureApplication(__FUNCTION__))
    {
      DALI_LOG_ERROR("WidgetContext::OnResize: EnsureApplication failed");
      return;
    }
  ...
  {
    DALI_LOG_RELEASE_INFO("WidgetContext::OnUpdate called (force:%d)\n", force ? 1 : 0);
    if(!EnsureApplication(__FUNCTION__))
    {
      DALI_LOG_ERROR("WidgetContext::OnUpdate: EnsureApplication failed");
      return;
    }
...
{
  DALI_LOG_RELEASE_INFO("RegisterWidgetCallback called (widgetName:%s, data:%p)\n", widgetName ? widgetName : "null", data);
  if(data == nullptr)
  {
    DALI_LOG_ERROR("RegisterWidgetCallback: data is null");
  }
...
{
  DALI_LOG_RELEASE_INFO("SetContentInfo called (handle:%p, bundleData:%p)\n", handle, bundleData);
