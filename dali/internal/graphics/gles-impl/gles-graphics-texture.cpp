...
bool Texture::InitializeNativeImage()
{
  ...
  bool                    created     = nativeImage->GlExtensionCreate();
  NativeImageInterface::Extension* extension = nativeImage->GetExtension();
  if( extension )
  {
    mGlTarget = extension->GetEglImageTextureTarget();
  }
  ...
  if(created)
  {
    ...
    if(nativeImage->TargetTexture() != 0u)
    {
      ...
      nativeImage->GlExtensionDestroy();
      texture = 0u;
      created = false;
    }
    else
...
void Texture::DestroyResource()
{
  ...
  if(mCreateInfo.nativeImagePtr)
  {
    mCreateInfo.nativeImagePtr->GlExtensionDestroy();
  }
}
