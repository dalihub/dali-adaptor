...
bool AreBoundsFinite(const Dali::Rect<float>& bounds)
{
...
Dali::Rect<float> IntersectBounds(const Dali::Rect<float>& lhs, const Dali::Rect<float>& rhs)
{
  if(!AreBoundsFinite(lhs) || !AreBoundsFinite(rhs))
  {
    return Dali::Rect<float>{};
  }
  ...
  return Dali::Rect<float>{left, top, right - left, bottom - top};
}
...
float ComputeVisibilityRatio(const Dali::Rect<float>& entityBounds, const Dali::Rect<float>& clipBounds)
{
...
void AppendAnnotatedEntities(Dali::Actor                      root,
                             const Dali::Rect<float>&              windowClipBounds,
                             ...
                             std::vector<EntityData>&         outEntityDataList)
{
  ...
  for(auto& candidate : candidates)
  {
    ...
    Dali::Rect<float> clipBounds = windowClipBounds;
...
bool EntityDataService::FindByActorId(const std::string& actorId, Dali::Actor root, EntityData& outEntityData)
{
  ...
  if(!found && GetActorId(root) == parsedActorId)
  {
    found = root;
  }
  if(!found)
...
bool EntityDataService::GetAnnotatedEntities(std::vector<EntityData>& outEntityDataList)
{
  ...
  for(auto& window : windows)
  {
    ...
    const Dali::Window::WindowSize windowSize = window.GetSize();
    Dali::Rect<float>              windowClipBounds;
    windowClipBounds.x      = 0.0f;
    windowClipBounds.y      = 0.0f;
    windowClipBounds.width  = static_cast<float>(windowSize.GetWidth());
    windowClipBounds.height = static_cast<float>(windowSize.GetHeight());
...
bool EntityDataService::GetAnnotatedEntities(Dali::Actor root, const Dali::Rect<float>& clipBounds, std::vector<EntityData>& outEntityDataList)
{
  outEntityDataList.clear();
  std::unordered_set<std::string> emittedActorIds;
