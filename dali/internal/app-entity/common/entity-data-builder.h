...
/**
 * @brief Returns the unique Actor identifier.
 *
 * Actor::GetId() is not part of the public API on this branch, so the ID
 * property is read instead.
 *
 * @param[in] actor The Actor to query
 * @return The Actor ID, or 0 for an empty handle
 */
DALI_ADAPTOR_API uint32_t GetActorId(Dali::Actor actor);
 ...
 */
DALI_ADAPTOR_API Dali::Rect<float> GetActorWindowBounds(Dali::Actor actor);
