...
// CLASS HEADER
#include <dali/internal/imaging/macos/native-image-factory-mac.h>
...
NativeImageFactoryCocoa::CreateNativeImage(
  ...
  Any                           nativeImage)
{
  return std::unique_ptr<NativeImage>(NativeImageCocoa::New(
    width,
    height,
    depth,
    nativeImage));
...
std::unique_ptr<NativeImageQueue>
NativeImageFactoryCocoa::CreateNativeImageQueue(
  uint32_t                            queueCount,
  uint32_t                            width,
  uint32_t                            height,
  Dali::NativeImageQueue::ColorFormat colorFormat,
  Any                                 nativeImageQueue)
{
