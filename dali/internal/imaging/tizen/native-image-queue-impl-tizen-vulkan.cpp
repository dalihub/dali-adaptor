...
NativeImageQueueTizenVulkan* NativeImageQueueTizenVulkan::New(uint32_t queueCount, uint32_t width, uint32_t height, Dali::NativeImageQueue::ColorFormat colorFormat, Any nativeImageQueue)
{
  NativeImageQueueTizenVulkan* image = new NativeImageQueueTizenVulkan(queueCount, width, height, nativeImageQueue);
  DALI_ASSERT_DEBUG(image && "NativeImageQueueTizenVulkan allocation failed.");
...
NativeImageQueueTizenVulkan::NativeImageQueueTizenVulkan(uint32_t queueCount, uint32_t width, uint32_t height, Any nativeImageQueue)
: mMutex(),
...
NativeImageQueueTizenVulkan::~NativeImageQueueTizenVulkan()
{
  if(mOwnTbmQueue)
  {
    if(mTbmQueue != nullptr)
...
void NativeImageQueueTizenVulkan::Initialize(Dali::NativeImageQueue::ColorFormat colorFormat)
{
  ...
  if(mTbmQueue == nullptr)
  {
    ...
    if(!mTbmQueue)
    {
      DALI_LOG_ERROR("NativeImageQueueTizenVulkan::Initialize: tbm_surface_queue_create is failed! [%p]\n", mTbmQueue);
      return;
    }
...
tbm_surface_queue_h NativeImageQueueTizenVulkan::GetSurfaceFromAny(Any source) const
{
  if(source.Empty())
  {
    return nullptr;
...
Any NativeImageQueueTizenVulkan::GetNativeImageQueue() const
{
  return Any(mTbmQueue);
}

void NativeImageQueueTizenVulkan::SetSize(uint32_t width, uint32_t height)
{
...
void NativeImageQueueTizenVulkan::IgnoreSourceImage()
{
  Dali::Mutex::ScopedLock lock(mMutex);
  ...
  if(tbm_surface_queue_can_acquire(mTbmQueue, 0))
  {
    ...
    {
      DALI_LOG_ERROR("NativeImageQueueTizenVulkan::IgnoreSourceImage: Failed to aquire a tbm_surface\n");
      return;
    }
...
bool NativeImageQueueTizenVulkan::CanDequeueBuffer()
{
  Dali::Mutex::ScopedLock lock(mMutex);
...
uint8_t* NativeImageQueueTizenVulkan::DequeueBuffer(uint32_t& width, uint32_t& height, uint32_t& stride, Dali::NativeImageQueue::BufferAccessType type)
{
  Dali::Mutex::ScopedLock lock(mMutex);
  if(mTbmQueue == nullptr)
  {
  ...
  int tbmOption = 0;
  if(type & Dali::NativeImageQueue::BufferAccessType::READ)
  {
    tbmOption |= TBM_OPTION_READ;
  }
  if(type & Dali::NativeImageQueue::BufferAccessType::WRITE)
  {
...
bool NativeImageQueueTizenVulkan::EnqueueBuffer(uint8_t* buffer)
{
  Dali::Mutex::ScopedLock lock(mMutex);
...
void NativeImageQueueTizenVulkan::CancelDequeuedBuffer(uint8_t* buffer)
{
  Dali::Mutex::ScopedLock lock(mMutex);
  auto                    bufferInstance = mBuffers.find(buffer);
  if(bufferInstance != mBuffers.end())
...
void NativeImageQueueTizenVulkan::FreeReleasedBuffers()
{
  Dali::Mutex::ScopedLock lock(mMutex);
  mFreeRequest = true;
}

bool NativeImageQueueTizenVulkan::CreateResource()
{
  return true;
}

void NativeImageQueueTizenVulkan::DestroyResource()
{
  Dali::Mutex::ScopedLock lock(mMutex);
...
uint32_t NativeImageQueueTizenVulkan::TargetTexture()
{
  return 0;
}

Dali::NativeImageInterface::PrepareTextureResult NativeImageQueueTizenVulkan::PrepareTexture()
{
  Dali::Mutex::ScopedLock lock(mMutex);
...
bool NativeImageQueueTizenVulkan::ApplyNativeFragmentShader(std::string& shader, int mask)
{
  return false;
}

const char* NativeImageQueueTizenVulkan::GetCustomSamplerTypename() const
{
  return nullptr;
}

int NativeImageQueueTizenVulkan::GetTextureTarget() const
{
  return 0;
}

Any NativeImageQueueTizenVulkan::GetNativeImageHandle() const
{
  //  return nullptr;
  return Any(mConsumeSurface);
}

bool NativeImageQueueTizenVulkan::SourceChanged() const
{
  return true;
}

void NativeImageQueueTizenVulkan::ResetSurfaceList(bool releaseConsumeSurface)
{
...
bool NativeImageQueueTizenVulkan::CheckBlending(int format)
{
...
void NativeImageQueueTizenVulkan::AcquireSurfaceReference(void* surface)
{
  if(!surface) return;
...
void NativeImageQueueTizenVulkan::ReleaseSurfaceReference(void* surface)
{
  if(!surface) return;
...
void NativeImageQueueTizenVulkan::ScheduleSurfaceForDelayedRelease(tbm_surface_h surface)
{
...
void NativeImageQueueTizenVulkan::CleanupPendingReleases()
{
...
NativeImageQueueTizenVulkan::SurfaceReferenceData& NativeImageQueueTizenVulkan::GetOrCreateSurfaceRef(tbm_surface_h surface)
{
  auto it = mSurfaceRefs.find(surface);
  if(it == mSurfaceRefs.end())
  {
