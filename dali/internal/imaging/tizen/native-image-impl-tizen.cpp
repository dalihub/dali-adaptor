...
NativeImageTizen* NativeImageTizen::New(uint32_t width, uint32_t height, Dali::NativeImage::ColorDepth depth, Any nativeImage)
{
  NativeImageTizen* image = new NativeImageTizen(width, height, depth, nativeImage);
...
NativeImageTizen::NativeImageTizen(uint32_t width, uint32_t height, Dali::NativeImage::ColorDepth depth, Any nativeImage)
: mWidth(width),
  mHeight(height),
  mTbmSurface(NULL),
...
void NativeImageTizen::Initialize()
{
  ...
  TbmSurfaceCounter::GetInstance().AddNativeImage();
...
tbm_surface_h NativeImageTizen::GetSurfaceFromAny(Any source) const
{
  if(source.Empty())
  {
    return NULL;
...
void NativeImageTizen::DestroySurface()
{
  if(mTbmSurface)
  {
    ...
    TbmSurfaceCounter::GetInstance().RemoveNativeImage();
  }
}

NativeImageTizen::~NativeImageTizen()
{
  {
...
Any NativeImageTizen::GetNativeImage() const
{
  return Any(mTbmSurface);
}

bool NativeImageTizen::GetPixels(Dali::Vector<uint8_t>& pixbuf, uint32_t& width, uint32_t& height, Pixel::Format& pixelFormat) const
{
  std::scoped_lock lock(mMutex);
  if(mTbmSurface != NULL)
  {
...
bool NativeImageTizen::SetPixels(uint8_t* pixbuf, const Pixel::Format& pixelFormat)
{
  std::scoped_lock lock(mMutex);
  if(mTbmSurface != NULL)
  {
...
void NativeImageTizen::SetSource(Any source)
{
  std::scoped_lock lock(mMutex);
...
bool NativeImageTizen::IsColorDepthSupported(Dali::NativeImage::ColorDepth colorDepth)
{
  uint32_t*  formats;
  uint32_t   formatNum;
  tbm_format format = TBM_FORMAT_RGB888;
...
bool NativeImageTizen::CreateResource()
{
...
void NativeImageTizen::DestroyResource()
{
  std::scoped_lock lock(mMutex);
  if(mEglImageKHR)
  {
...
uint32_t NativeImageTizen::TargetTexture()
{
...
Dali::NativeImageInterface::PrepareTextureResult NativeImageTizen::PrepareTexture()
{
  std::scoped_lock lock(mMutex);
  if(mSetSource)
  {
...
bool NativeImageTizen::ApplyNativeFragmentShader(std::string& shader, int mask)
{
  return mEglGraphics->ApplyNativeFragmentShader(shader, SAMPLER_TYPE, mask);
}

const char* NativeImageTizen::GetCustomSamplerTypename() const
{
  return SAMPLER_TYPE;
}

int NativeImageTizen::GetTextureTarget() const
{
  return GL_TEXTURE_EXTERNAL_OES;
}

Any NativeImageTizen::GetNativeImageHandle() const
{
  return GetNativeImage();
}

bool NativeImageTizen::SourceChanged() const
{
  std::scoped_lock lock(mMutex);
  if(mTbmBackSurface)
  {
...
Rect<uint32_t> NativeImageTizen::GetUpdatedArea()
{
  std::scoped_lock lock(mMutex);
...
bool NativeImageTizen::CheckBlending(tbm_format format)
{
...
uint8_t* NativeImageTizen::AcquireBuffer(uint32_t& width, uint32_t& height, uint32_t& stride)
{
...
bool NativeImageTizen::ReleaseBuffer(const Rect<uint32_t>& updatedArea)
{
  bool ret = false;
  if(mTbmSurface != NULL)
  {
...
void NativeImageTizen::SetResourceDestructionCallback(EventThreadCallback* callback, bool ownedCallback)
{
  std::scoped_lock lock(mMutex);
  if(mOwnResourceDestructionCallback)
  {
...
void NativeImageTizen::EnableBackBuffer(bool enable)
{
  std::scoped_lock lock(mMutex);
...
void NativeImageTizen::CreateBackBuffer()
{
...
void NativeImageTizen::DestroyBackBuffer()
{
  if(mTbmBackSurface)
  {
...
void NativeImageTizen::PostRender()
{
  mEglImageChanged = false;
}
} // namespace Adaptor
