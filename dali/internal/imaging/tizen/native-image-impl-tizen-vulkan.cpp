...
// CLASS HEADER
#include <dali/internal/imaging/tizen/native-image-impl-tizen-vulkan.h>
...
NativeImageTizenVulkan* NativeImageTizenVulkan::New(uint32_t width, uint32_t height, Dali::NativeImage::ColorDepth depth, Any nativeImage)
{
  NativeImageTizenVulkan* image = new NativeImageTizenVulkan(width, height, depth, nativeImage);
...
NativeImageTizenVulkan::NativeImageTizenVulkan(uint32_t width, uint32_t height, Dali::NativeImage::ColorDepth depth, Any nativeImage)
: mWidth(width),
  mHeight(height),
  mColorDepth(depth),
  mTbmSurface(nullptr),
...
void NativeImageTizenVulkan::Initialize()
{
...
tbm_surface_h NativeImageTizenVulkan::GetSurfaceFromAny(Any source) const
{
  if(source.Empty())
  {
    return nullptr;
...
void NativeImageTizenVulkan::DestroySurface()
{
  if(mTbmSurface)
  {
...
NativeImageTizenVulkan::~NativeImageTizenVulkan()
{
  {
...
Any NativeImageTizenVulkan::GetNativeImage() const
{
  return Any(mTbmSurface);
}

bool NativeImageTizenVulkan::GetPixels(Dali::Vector<uint8_t>& pixbuf, uint32_t& width, uint32_t& height, Pixel::Format& pixelFormat) const
{
  std::scoped_lock lock(mMutex);
  if(mTbmSurface != nullptr)
  {
...
bool NativeImageTizenVulkan::SetPixels(uint8_t* pixbuf, const Pixel::Format& pixelFormat)
{
  std::scoped_lock lock(mMutex);
  if(mTbmSurface != nullptr)
  {
...
void NativeImageTizenVulkan::SetSource(Any source)
{
  std::scoped_lock lock(mMutex);
...
bool NativeImageTizenVulkan::IsColorDepthSupported(Dali::NativeImage::ColorDepth colorDepth)
{
  uint32_t*  formats;
  uint32_t   formatNum;
  tbm_format format = TBM_FORMAT_RGB888;
...
bool NativeImageTizenVulkan::CreateResource()
{
  std::scoped_lock lock(mMutex);
...
void NativeImageTizenVulkan::DestroyResource()
{
  std::scoped_lock lock(mMutex);
...
uint32_t NativeImageTizenVulkan::TargetTexture()
{
...
Dali::NativeImageInterface::PrepareTextureResult NativeImageTizenVulkan::PrepareTexture()
{
  std::scoped_lock lock(mMutex);
...
bool NativeImageTizenVulkan::ApplyNativeFragmentShader(std::string& shader, int mask)
{
  // Not used in Vulkan backend
  return false;
}

const char* NativeImageTizenVulkan::GetCustomSamplerTypename() const
{
  return nullptr;
}

int NativeImageTizenVulkan::GetTextureTarget() const
{
  // Not used in Vulkan backend
  return 0;
}

Any NativeImageTizenVulkan::GetNativeImageHandle() const
{
  return GetNativeImage();
}

bool NativeImageTizenVulkan::SourceChanged() const
{
  std::scoped_lock lock(mMutex);
  if(mTbmBackSurface)
  {
...
Rect<uint32_t> NativeImageTizenVulkan::GetUpdatedArea()
{
  std::scoped_lock lock(mMutex);
...
bool NativeImageTizenVulkan::CheckBlending(tbm_format format)
{
...
uint8_t* NativeImageTizenVulkan::AcquireBuffer(uint32_t& width, uint32_t& height, uint32_t& stride)
{
...
bool NativeImageTizenVulkan::ReleaseBuffer(const Rect<uint32_t>& updatedArea)
{
  bool ret = false;
...
void NativeImageTizenVulkan::SetResourceDestructionCallback(EventThreadCallback* callback, bool ownedCallback)
{
  std::scoped_lock lock(mMutex);
  if(mOwnResourceDestructionCallback)
  {
...
void NativeImageTizenVulkan::EnableBackBuffer(bool enable)
{
  std::scoped_lock lock(mMutex);
...
void NativeImageTizenVulkan::CreateBackBuffer()
{
...
void NativeImageTizenVulkan::DestroyBackBuffer()
{
  if(mTbmBackSurface)
  {
