...
NativeImageQueueTizen* NativeImageQueueTizen::New(uint32_t queueCount, uint32_t width, uint32_t height, Dali::NativeImageQueue::ColorFormat colorFormat, Any nativeImageQueue)
{
  NativeImageQueueTizen* image = new NativeImageQueueTizen(queueCount, width, height, nativeImageQueue);
  DALI_ASSERT_DEBUG(image && "NativeImageQueueTizen allocation failed.");
...
NativeImageQueueTizen::NativeImageQueueTizen(uint32_t queueCount, uint32_t width, uint32_t height, Any nativeImageQueue)
: mMutex(),
...
NativeImageQueueTizen::~NativeImageQueueTizen()
{
  // Remove from counter before destroying queue
  if(mTbmQueue)
  {
    TbmSurfaceCounter::GetInstance().RemoveNativeImageQueue(mQueueCount);
...
void NativeImageQueueTizen::Initialize(Dali::NativeImageQueue::ColorFormat colorFormat)
{
  ...
  if(mTbmQueue == nullptr)
  {
    ...
    if(!mTbmQueue)
    {
      DALI_LOG_ERROR("NativeImageQueueTizen::Initialize: tbm_surface_queue_create is failed! [%p]\n", mTbmQueue);
      return;
    }
  ...
  TbmSurfaceCounter::GetInstance().AddNativeImageQueue(mQueueCount);
}

tbm_surface_queue_h NativeImageQueueTizen::GetSurfaceFromAny(Any source) const
{
  if(source.Empty())
  {
    return nullptr;
...
Any NativeImageQueueTizen::GetNativeImageQueue() const
{
  return Any(mTbmQueue);
}

void NativeImageQueueTizen::SetSize(uint32_t width, uint32_t height)
{
...
void NativeImageQueueTizen::IgnoreSourceImage()
{
  Dali::Mutex::ScopedLock lock(mMutex);
  ...
  if(tbm_surface_queue_can_acquire(mTbmQueue, 0))
  {
    ...
    {
      DALI_LOG_ERROR("NativeImageQueueTizen::IgnoreSourceImage: Failed to aquire a tbm_surface\n");
      return;
    }
...
bool NativeImageQueueTizen::CanDequeueBuffer()
{
  Dali::Mutex::ScopedLock lock(mMutex);
...
uint8_t* NativeImageQueueTizen::DequeueBuffer(uint32_t& width, uint32_t& height, uint32_t& stride, Dali::NativeImageQueue::BufferAccessType type)
{
...
bool NativeImageQueueTizen::EnqueueBuffer(uint8_t* buffer)
{
  Dali::Mutex::ScopedLock lock(mMutex);
...
void NativeImageQueueTizen::CancelDequeuedBuffer(uint8_t* buffer)
{
  Dali::Mutex::ScopedLock lock(mMutex);
  auto                    bufferInstance = mBuffers.find(buffer);
  if(bufferInstance != mBuffers.end())
...
void NativeImageQueueTizen::FreeReleasedBuffers()
{
  Dali::Mutex::ScopedLock lock(mMutex);
  mFreeRequest = true;
}

void NativeImageQueueTizen::SetQueueUsageHint(Dali::NativeImageQueue::QueueUsageType type)
{
  mWaitInWorkerThread = (type == Dali::NativeImageQueue::QueueUsageType::ENQUEUE_DEQUEUE) ? true : false;
}

bool NativeImageQueueTizen::CreateResource()
{
...
void NativeImageQueueTizen::DestroyResource()
{
  Dali::Mutex::ScopedLock lock(mMutex);
...
uint32_t NativeImageQueueTizen::TargetTexture()
{
  Dali::Mutex::ScopedLock lock(mMutex);
...
Dali::NativeImageInterface::PrepareTextureResult NativeImageQueueTizen::PrepareTexture()
{
...
bool NativeImageQueueTizen::ApplyNativeFragmentShader(std::string& shader, int mask)
{
  return mEglGraphics->ApplyNativeFragmentShader(shader, SAMPLER_TYPE, mask);
}

const char* NativeImageQueueTizen::GetCustomSamplerTypename() const
{
  return SAMPLER_TYPE;
}

int NativeImageQueueTizen::GetTextureTarget() const
{
  return GL_TEXTURE_EXTERNAL_OES;
}

Any NativeImageQueueTizen::GetNativeImageHandle() const
{
  return nullptr;
}

bool NativeImageQueueTizen::SourceChanged() const
{
  return true;
}

// This method should be called on the render thread.
bool NativeImageQueueTizen::CreateSyncObject()
{
  tbm_surface_h tbmSurface;
...
void NativeImageQueueTizen::PostRender()
{
  Dali::Mutex::ScopedLock lock(mMutex);
...
void NativeImageQueueTizen::ResetEglImageList(bool releaseConsumeSurface)
{
...
bool NativeImageQueueTizen::CheckBlending(int format)
{
