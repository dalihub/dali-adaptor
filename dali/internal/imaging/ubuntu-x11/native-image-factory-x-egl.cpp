...
// CLASS HEADER
#include <dali/internal/imaging/ubuntu-x11/native-image-factory-x.h>
...
{
  return std::unique_ptr<NativeImage>(NativeImageX::New(width, height, depth, nativeImage));
}
...
{
  return std::unique_ptr<NativeImageQueue>(NativeImageQueueX::New(queueCount, width, height, colorFormat, nativeImageQueue));
}
