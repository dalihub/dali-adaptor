...
public:
  ...
  void Say(const std::string& text, bool discardable, std::function<void(std::string)> callback) override
  {
    ...
    mDirectReadingClient.method<DBus::ValueOrError<std::string, bool, int32_t>(std::string, bool)>("ReadCommand").asyncCall([=, this](DBus::ValueOrError<std::string, bool, int32_t> msg)
    {
      if(!msg)
      {
        LOG() << "Direct reading command failed (" << msg.getError().message << ")\n";
  ...
  {
    AddCoalescableMessage(CoalescableMessages::SET_OFFSET, socket, 1.0f, [=, this]()
    {
